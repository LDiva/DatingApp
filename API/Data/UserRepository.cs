...
namespace API.Data
{
    public class UserRepository : IUserRepository
    {
        ...
        public async Task<PagedList<MemberDto>> GetMembersAsync(UserParms userParms)
        {
           ...
           var minDob = DateOnly.FromDateTime(DateTime.Today.AddYears(-userParms.MaxAge - 1));
           var maxDob = DateOnly.FromDateTime(DateTime.Today.AddYears(-userParms.MinAge));

            query = query.Where(u => u.DateOfBirth >= minDob && u.DateOfBirth <= maxDob);
           
           query = userParms.OrederBy switch
           {
